        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.15fr_0.85fr] gap-8 lg:gap-10">
            ...
            <div className="space-y-4 sm:space-y-6">
              {experiences.map((exp, idx) => (
                <FadeIn key={exp.company} delay={0.2 + idx * 0.1} direction="up" distance={20}>
                  <div className="group rounded-3xl border border-white/10 bg-[#0e121a]/90 p-5 sm:p-7 lg:p-8 backdrop-blur-xl transition-all duration-300 hover:border-[#14b8a6]/40 hover:bg-[#111722]">
                    <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                      <h4 className="font-display text-lg sm:text-xl font-bold text-white group-hover:text-[#14b8a6] transition-colors">
                        {exp.role}
                      </h4>
                      <span className="rounded-full border border-[#14b8a6]/20 bg-[#14b8a6]/10 px-2.5 py-0.5 font-mono text-xs text-[#14b8a6]">
                        {exp.period}
                      </span>
                    </div>

                    <div className="text-xs sm:text-sm font-medium text-white/70 mb-1">{exp.company}</div>
                    <div className="text-[11px] sm:text-xs text-[#64748b] mb-4">{exp.type} • {exp.location}</div>

                    <p className="text-xs sm:text-sm leading-relaxed text-[#94a3b8] mb-4">
                      {exp.description}
                    </p>

                    <ul className="space-y-2">
                      {exp.highlights.map((h, i) => (
                        <li key={i} className="flex items-start gap-2 text-xs sm:text-sm text-[#94a3b8] leading-relaxed">
          ...
          <div>
            ...
            <FadeIn delay={0.35} direction="up" distance={20}>
              <div className="rounded-3xl border border-white/10 bg-[#0e121a]/90 p-5 sm:p-7 lg:p-8 backdrop-blur-xl transition-all duration-300 hover:border-[#14b8a6]/40 hover:bg-[#111722] mb-6">
                <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                  <h4 className="font-display text-xl font-bold text-white">
                    {education.shortDegree}
                  </h4>
                  <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 font-mono text-xs text-[#94a3b8]">
                    {education.start} – {education.end}
                  </span>
                </div>

                <div className="text-sm font-medium text-white/70 mb-1">{education.institution}</div>
                <div className="text-xs text-[#64748b] mb-4">{education.location}</div>
            ...
            <FadeIn delay={0.5} direction="up" distance={20}>
              <div className="rounded-2xl border border-white/5 bg-white/[0.02] p-6 text-left">
                ...
                <p className="text-xs sm:text-sm text-[#94a3b8] leading-relaxed">
                  Dedicated to modular architecture, type-safe API communication, robust error-handling pipelines, and maintaining 60fps frame pacing without memory leaks.
                </p>
              </div>
            </FadeIn>
          </div>
